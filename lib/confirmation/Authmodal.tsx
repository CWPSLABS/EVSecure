...
interface AuthModalProps {
  ...
  onClose: () => void;
  onSuccess: () => void;
  context?: 'payment' | 'sync'; // NEW: context prop
}

export function AuthModal({ isOpen, onClose, onSuccess, context }: AuthModalProps) {
  const [mode, setMode] = useState<'signin' | 'signup' | 'reset'>('signup'); // default to signup for new users
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'reset') {
        ...
        // If context is payment, show specific message
        if (context === 'payment') {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="bg-slate-800 rounded-xl p-6 max-w-md w-full border border-slate-700 shadow-2xl">
        ...
        {/* Context banner - shown when opening from Subscribe button */}
        {context === 'payment' && (
