...
import { AuthModal } from '@/lib/confirmation/Authmodal'; // ← ADD THIS
import { 
  PAYSTACK_PUBLIC_KEY, 
  PLANS, 
  getPlanPrice, 
...
export default function PricingPage() {
  ...
  const [isAuthModalOpen, setIsAuthModalOpen] = useState(false); // ← ADD THIS
  ...
  const handleSubscribe = async () => {
    console.log('🔴 SUBSCRIBE CLICKED - isAuth:', isAuth, 'email:', userEmail);

    // If not signed in, open auth modal directly on this page
    if (!isAuth || !userEmail) {
    ...
    console.log('✅ Signed in - starting payment...');
    setLoading(true);
    ...
    const config = {
      ...
      onSuccess: async (response: any) => {
        ...
        try {
          ...
          alert(`Payment successful but activation failed. Contact support with reference: ${response.reference}`);
        } finally {
          setLoading(false);
        }
      },
  ...
  // ← ADD: After sign in success, re-check auth then trigger payment
  const handleAuthSuccess = async () => {
    ...
    // Small delay then trigger payment
    setTimeout(() => {
      console.log('🚀 Triggering payment after sign in...');
      handleSubscribe();
    }, 1000);
  };
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white">
            ...
            <nav className="flex items-center gap-4">
              ...
              {!isAuth && (
                <button
                  ...
                  className="px-4 py-2 text-sm border border-slate-700/50 rounded-lg hover:bg-slate-800/50 transition-colors"
                >
                  Sign In
                </button>
              )}
      ...
      {/* Rest of your pricing page stays exactly the same */}
      <div className="max-w-7xl mx-auto px-6 py-16">
          ...
          <div className="relative bg-gradient-to-br from-purple-600/20 to-blue-600/20 backdrop-blur-sm rounded-2xl p-8 border-2 border-purple-500 hover:border-purple-400 transition-colors">
            ...
            >
              {loading ? (
                ...
                'Sign In to Subscribe'  // ← Better label when not signed in
              ) : (
                'Subscribe Now'
              )}
            </button>
        ...
        {/* FAQ Section - keep exactly as is */}
